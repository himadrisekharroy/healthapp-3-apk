		...
		console.log(data);
	 	...
	 	if(data.success)
	 	{
	 		...
	 		for(var key in data.data)
			{
				...
				html_str +=			           	'</div>';
				html_str +=			           	'<div class="col-2" style="border-left:1px solid #d64872">';
				html_str +=							'<a href="tel:'+data.data[key].mobile +'" class="call_link"><i class="fa fa-phone" aria-hidden="true"></i><br><small>Call</small></a>'
				html_str +=			           '</div>';
				html_str +=			       	'</div>';
	    	...
	    	for(var key in data.upcomming.data)
		 	{
			    ...
			    appt_html +='       		<small>'+data.upcomming.data[key].age+' '+data.upcomming.data[key].gender+'</small><br/>';	
			    appt_html +='       	</div>';
			    appt_html +='       	<div class="col-3 v-call" style="border-left:1px solid #d64872">';
			   	appt_html +=				'<a href="tel:'+data.upcomming.data[key].mobile +'" class="call_link"><i class="fa fa-phone" aria-hidden="true"></i><br><small>Ph. Call</small></a>'
			    appt_html +='				<a href="javascript:void(0)" style="margin-top: 10px;" onclick="video_chat_call_doctor('+counter+', '+data.upcomming.data[key].id+')" class="call_link"><i class="fa fa-video-camera" aria-hidden="true"></i><br><small>Video Call</small></a>';
			    appt_html +='       	</div>';
			    ...
			    appt_html +='   			<b style="font-size:20px;">'+data.upcomming.data[key].time+'</b><div style="margin-top:-10px;"><small>'+months_arr[parseInt(data.upcomming.data[key].month)-1]+' '+data.upcomming.data[key].day+', '+data.upcomming.data[key].year+'</small></div>';
			    appt_html +='   		</div>';							       		
			    appt_html +='   	</div>';
	    	...
	    	for(var key in data.previous.data)
		 	{
		 		...
		 		var user_age_gender = data.previous.data[key].age+' '+data.previous.data[key].gender;
			    ...
			    appt_html +='       	</div>';
			    appt_html +='       	<div class="col-3 v-call" style="border-left:1px solid #d64872">';
			   	appt_html +=				'<a href="tel:'+data.previous.data[key].mobile +'" class="call_link"><i class="fa fa-phone" aria-hidden="true"></i><br><small>Call</small></a>'
			    appt_html +='       	</div>';
			    appt_html +='   	</div>';
			    appt_html +='   	<div class="row doctors-link">';
			    appt_html +='			<div class="col-2"><img src="images/app_book_confirm.png" style="width:50px;"/></div>';
			    appt_html +='   		<div class="col-5">';
			    appt_html +='   			<b style="font-size:20px;">'+data.previous.data[key].time+'</b><div style="margin-top:-10px;"><small>'+months_arr[parseInt(data.previous.data[key].month)-1]+' '+data.previous.data[key].day+', '+data.previous.data[key].year+'</small></div>';
			    appt_html +='   		</div>';
...
function video_chat_call_doctor(counter, apt_id)
{
	//navigator.notification.beep(2);
	window.location.href="video_call_temp.html?&appt_id="+apt_id+"&user_id="+storage.getItem("logged_user_id")+"&type=d";
}

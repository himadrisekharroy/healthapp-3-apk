					...
					for(var key in data.data)
		 			{
		 				...
		 				search_arg_arr[i]['arg1'] = data.data[key].mobile;
						search_arg_arr[i]['arg2'] = data.data[key].ph_time.str;

						var call_btn_link = '<a href="javascript:void(0)" onclick="conf_bfor_call('+i+',\'search\')" class="call_link"><i class="fa fa-phone" aria-hidden="true"></i><br><small>Call</small></a>';
...
{
	var str = $("#year_id_"+counter).val()+"-"+(parseInt($("#month_id_"+counter).val()))+"-"+ $("#day_id_"+counter).val() +" "+ $("#time_id_"+counter).val();
	...
	else
	{
		...
		window.location.href="video_call_temp.html?&appt_id="+$("#appt_id_"+counter).val()+"&user_id="+storage.getItem("logged_user_id")+"&type=u";
		// initiate call
		...
		.done(function(data){
	 	...
	 	else
	 	{
	 		...
	 		for(var key in data.data)
		 	{
				...
				load_arg_arr[i]['arg1'] = data.data[key].mobile;
				load_arg_arr[i]['arg2'] = data.data[key].ph_time.str;

				var call_btn_link = '<a href="javascript:void(0)" onclick="conf_bfor_call('+i+',\'load\')" class="call_link"><i class="fa fa-phone" aria-hidden="true"></i><br><small>Call</small></a>';
...
{
	window.open("http://www.demo.ritzcybernetics.com/health_app/doctor-prescription.php?" + $("#base_pres").val(), '_system'); 
	//window.open("http://remotehealth.org/doctor-prescription.php?" + base_pres, '_system'); 
	return false;
}

function conf_bfor_call(i, type)
{
	//console.log(load_arg_arr[i]['arg1']);
	//console.log(load_arg_arr[i]['arg2']);
	var msg = "";
	var ph = "";
	if(type == "load")
	{
		ph = load_arg_arr[i]['arg1'];
		msg = load_arg_arr[i]['arg2'];
	}
	else if(type == "search")
	{
		ph = search_arg_arr[i]['arg1'];
		msg = search_arg_arr[i]['arg2'];
	}

	var r = confirm("Time for call the Doctor\n"+ msg  +"\n Are you sure to call?");
	if (r == true) {

var api_url="http://www.demo.ritzcybernetics.com/health_app/api/";
var site_url="http://www.demo.ritzcybernetics.com/health_app/";
...
jQuery(document).ready(function($) {
	...
	else if(storage.getItem("logged_user_type") == 'n')
	{
		$("#user_name_show").html("<br/>"+storage.getItem("nurse_f_name")+" "+storage.getItem("nurse_l_name"));
	...
	$("#tans_to_conptime").click(function(){
		slideBabyRight("phone_call_timimg51.html");
	})
	...
	$("#nurse_login_form").submit(function(e){
		e.preventDefault();
		...
		$.post( api_url+"get_doctor_details.php", 
		  ...
		  .done(function( data ) {
		    ...
		    if(data.success)
		    {
		    	...
		    	localStorage.setItem('logged_user_l_name', data.l_name);
		    	localStorage.setItem('logged_user_image', data.image);
		    	localStorage.setItem('logged_user_type','n');
		    	localStorage.setItem('logged_user_sex', data.sex);
		    	localStorage.setItem('logged_user_email', $("#user_login_email").val());
		    	localStorage.setItem('logged_user_dob', data.dob);
		    	localStorage.setItem('logged_user_ph', data.mobile);

		    	localStorage.setItem('nurse_id', data.nurse_id);
		    	localStorage.setItem('nurse_f_name', data.nurse_f_name);
		    	localStorage.setItem('nurse_l_name', data.nurse_l_name);
		    	localStorage.setItem('nurse_email_id', data.nurse_email_id);
		    	localStorage.setItem('nurse_mobile', data.nurse_mobile);
		    	localStorage.setItem('nurse_sex', data.nurse_sex);
	...
	$("#otp_validation_btn").click(function(){
		...
		insert_user_id = $("#insert_user_id").val();
			...
			{ 
				...
				otp: $("#otp").val()
	     		...
	     		.done(function(data){
	        		...
	        		if(data.success)
			    	{
			    		$(".alert-success").html("<strong>Success:</strong> Verification Successful. Please login...</strong>").slideDown();
			    		$("#succ_u_login_redirect").show();		    		
			    		$("#form_otp").hide();
			    	}
			    	else
		    	...
		    	for(var key in data.upcomming.data)
			 	{
				    ...
				    appt_html +='   			<div style="margin-top:-5px;"><b style="font-size:16px;">'+data.upcomming.data[key].time+'</b></div><div style="margin-top:-10px;"><small>'+months_arr[parseInt(data.upcomming.data[key].month)-1]+' '+data.upcomming.data[key].day+'</small></div><div style="margin-top:-8px"><small>'+data.upcomming.data[key].year+'</small></div>';
				    appt_html +='   		</div>';
